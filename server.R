...
server <- function(input, output, session) {
    ...
    pca_results_raw <- reactive({
        reducedDim(spe_pca(), "PCA")
    })
    
    output$pca_before <- renderPlot({
        req(input$filter_submit)
        drawPCA(spe_pca(), precomputed = pca_results_raw(), 
                color = colData(spe_pca())[,input$factors_of_interest_biology[1]], 
                shape = colData(spe_pca())[,input$factors_of_interest_biology[2]]) +
                labs(color = input$factors_of_interest_biology[1], shape = input$factors_of_interest_biology[2])
    ...
    output$pca_after <- renderPlot({
        req(input$filter_submit)
        drawPCA(spe_norm_pca(), precomputed = pca_results_norm(), 
                color = colData(spe_norm_pca())[,input$factors_of_interest_biology[1]], 
    ...
    output$pca_batch <- renderPlot({
        ...
        drawPCA(spe_batch_pca(), precomputed = pca_results_batch(), 
                color = colData(spe_batch_pca())[,input$factors_of_interest_biology[1]], 
    ...
    output$mds <- renderPlot({
        req(input$nor_submit)
        input$dim_submit
        isolate(standR::plotMDS(spe_batch_pca(), assay = 2, 
                        color = colData(spe_batch_pca())[,input$factor_of_interest_color],
                        shape = colData(spe_batch_pca())[,input$factor_of_interest_shape]) +
                        labs(color = input$factor_of_interest_color, shape = input$factor_of_interest_shape))
    })
    ...
    output$umap <- renderPlot({
        req(input$nor_submit)
        input$dim_submit
        isolate(plotDR(spe_batch_umap(), dimred = "UMAP", 
    ...
    output$tsne <- renderPlot({
        req(input$nor_submit)
        input$dim_submit
        isolate(plotDR(spe_batch_tsne(), dimred = "TSNE", 
    ...
    contr_number <- reactive({
      paste(gsub(" ","_",input$contrast_1), "-", gsub(" ","_",input$contrast_2))
    ...
    de_genes_table <- reactive({
      topTable(efit(), sort.by = "P", n = Inf, p.value = input$p_value, lfc = input$logfc)
    })
    ...
    output$de_table <- renderDataTable({
      ...
      datatable(de_genes_table()[,c("logFC", "AveExpr", "P.Value", "adj.P.Val")], caption = paste(input$contrast_1,"vs.",input$contrast_2))
    })
    ...
    output$bar_plot <- renderPlot({
      barplot(go_gsea(), showCategory=15)
    })
