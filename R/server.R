...
server <- function(input, output, session) {
  ...
  output$heatmap_seurat <- renderPlot({
    req(input$filter_submit_seurat)
    tissue_markers <- FindAllMarkers(tissue_umap(), only.pos = TRUE, min.pct = 0.2, logfc.threshold = 0.5, return.thresh = 0.05)
    tissue_markers %>% group_by(cluster) %>% top_n(n = 8, wt = avg_log2FC) -> topgene
  ...
  # perform differential expression analysis
  de_markers <- reactive({
    req(input$expre_submit_seurat)
    input$expre_submit_seurat
    isolate(FindMarkers(tissue_umap(), ident.1 = input$cluster_1, ident.2 = input$cluster_2,
  ...
  output$dot_plot_seurat <- renderPlot({
    req(input$gsea_submit_seurat)
    dotplot(go_gsea_seurat(), showCategory = input$category_dot_seurat)
  })
  ...
  output$network_seurat <- renderPlot({
    req(input$gsea_submit_seurat)
    cnetplot(go_gsea_seurat(), showCategory = input$category_network_seurat, foldChange = geneList_seurat(), circular = TRUE, colorEdge = TRUE)
  })


  # ridgeline plot
  output$ridgeline_plot_seurat <- renderPlot({
    req(input$gsea_submit_seurat)
    ridgeplot(go_gsea_seurat(), showCategory = input$category_ridgeline_seurat)
  })


  # running score and preranked list plot
  output$score_plot_seurat <- renderPlot({
    req(input$gsea_submit_seurat)
    gseaplot2(go_gsea_seurat(), geneSetID = 1:input$category_score_seurat)
  })
